...
/// What a WAL replay hands back to the collection: one memtable per log file,
/// **newest first** (the order `Collection::get` and `State::frozen` expect), plus the
/// highest sequence number seen, so writes resume above every record already on disk.
...
/// Ids of the log files in `dir`, ascending. Anything that is not a plain number is
/// ignored rather than failing the whole open (temp files, editor droppings, `.DS_Store`).
...
async fn latest_id<P: AsRef<Path>>(dir: P) -> DbResult<u64> {
    Ok(wal_ids(dir).await?.last().copied().unwrap_or(0))
}

/// Readers ordered newest log first; `read_dir` order is arbitrary and would let an
/// older memtable shadow a newer one during lookups.
...
mod tests {
    ...
    async fn restore_is_newest_log_first() {
        ...
        // Same user key in three logs; the highest id holds the newest version.
        for id in 0..3u64 {
            let writer = WalWriter::open(path.join(format!("{}", id))).await.unwrap();
            let key = Key::new("k", id + 1);
            writer.append(&key, &Value::set(&format!("v{id}"))).await.unwrap();
        }
        // Not a log file: must be skipped, not parsed as an id.
    ...
    #[tokio::test]
    async fn new_creates_missing_dir() {
        let dir = tempdir().unwrap();
